...
let lastSolanaTokensModified = 0;
...
function loadAllTokens(): { tokens: TokenWithChain[], solanaCount: number, ethereumCount: number } {
  ...
  if (fs.existsSync(SOLANA_TOKENS)) {
    try {
      const solanaData = JSON.parse(fs.readFileSync(SOLANA_TOKENS, 'utf8'));
      const solanaTokens = solanaData.tokens?.map((token: any) => ({
        ...token,
        chain: 'solana'
      })) || [];
      allTokens = allTokens.concat(solanaTokens);
  ...
  return { tokens: allTokens, solanaCount, ethereumCount };
}
...
function checkTokensUpdated(): boolean {
  ...
  try {
    if (fs.existsSync(SOLANA_TOKENS)) {
      const solanaStats = fs.statSync(SOLANA_TOKENS);
      const currentSolanaModified = solanaStats.mtimeMs;
      if (currentSolanaModified > lastSolanaTokensModified) {
        lastSolanaTokensModified = currentSolanaModified;
        updated = true;
      }
    }
...
async function main() {
  ...
  let tweetsObj: any = {};
  let tokenData: { tokens: TokenWithChain[], solanaCount: number, ethereumCount: number };
  ...
  try {
    if (fs.existsSync(SOLANA_TOKENS)) {
      const solanaStats = fs.statSync(SOLANA_TOKENS);
      lastSolanaTokensModified = solanaStats.mtimeMs;
    }
    if (fs.existsSync(ETHEREUM_TOKENS)) {
      const ethereumStats = fs.statSync(ETHEREUM_TOKENS);
  ...
  const solanaAnalyzed = analysisResults.filter(r => r.chain === 'solana').length;
