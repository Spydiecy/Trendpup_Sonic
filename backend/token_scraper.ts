...
async function scraper() {
  ...
  try {
    ...
    const solanaTokens = allTokens.filter(token => token.chain === 'solana');
    const ethereumTokens = allTokens.filter(token => token.chain === 'ethereum');
    ...
    const ethereumData = {
      timestamp: new Date().toISOString(),
      totalTokens: ethereumTokens.length,
      chain: 'ethereum',
      sortedBy: 'pairAge (ascending)',
      tokens: ethereumTokens
    };
    
    fs.writeFileSync('solana_tokens.json', JSON.stringify(solanaData, null, 2));
    fs.writeFileSync('ethereum_tokens.json', JSON.stringify(ethereumData, null, 2));
