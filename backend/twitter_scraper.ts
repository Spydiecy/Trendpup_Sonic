...
function getAllTokens(): Token[] {
  const tokens: Token[] = [];
  try {
    const sonicPairsPath = path.join(__dirname, '..', 'data', 'sonic_tokens.json');
    console.log(`Looking for sonic pairs at: ${sonicPairsPath}`);
    console.log(`File exists: ${fs.existsSync(sonicPairsPath)}`);
    
    if (fs.existsSync(sonicPairsPath)) {
      const sonicData = JSON.parse(fs.readFileSync(sonicPairsPath, 'utf8'));
      console.log(`Loaded ${sonicData.tokens?.length || 0} tokens from sonic_tokens.json`);
      
      // Parse Sonic tokens data structure (new format from token scraper)
      if (sonicData.tokens && Array.isArray(sonicData.tokens)) {
        for (const token of sonicData.tokens) {
        ...
        console.log('Warning: sonic_tokens.json is not in expected format');
      }
      console.log(`Extracted ${tokens.length} tokens from pairs`);
    } else {
      console.error(`Sonic pairs file not found at: ${sonicPairsPath}`);
    }
  } catch (error) {
...
const currentSonicHash = getFileHash(path.join(__dirname, '..', 'data', 'sonic_tokens.json'));

if (currentQueue.lastSonicFileHash === currentSonicHash && 
    currentQueue.activeTokens.length > 0) {
    console.log('No changes detected in token list');
    return currentQueue.activeTokens;
  }

  console.log('Updating scraping queue...');
  ...
  if (currentQueue.activeTokens.length === 0) {
    ...
    if (currentQueue.activeTokens.length > 0) {
      console.log('First few tokens:', currentQueue.activeTokens.slice(0, 3).map(t => t.symbol).join(', '));
    }
  }

  currentQueue.timestamp = new Date().toISOString();
  currentQueue.lastSonicFileHash = currentSonicHash;
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (fs.existsSync(path.join(__dirname, '..', 'data', 'cookies.json'))) {
        const cookiesRaw = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'data', 'cookies.json'), 'utf8'));
...
let lastSonicTokenSymbols = new Set();

try {
  const allTokens = getAllTokens();
  lastSonicTokenSymbols = new Set(allTokens.map((t: Token) => t.symbol));
} catch (error) {
}

const sonicPairsPath = path.join(__dirname, '..', 'data', 'sonic_tokens.json');
if (fs.existsSync(sonicPairsPath)) {
  fs.watchFile(sonicPairsPath, { interval: 5000 }, async (curr, prev) => {
    try {
      const allTokens = getAllTokens();
      const currentSonicSymbols = new Set(allTokens.map((t: Token) => t.symbol));
      const newSonicTokens = allTokens.filter((t: Token) => !lastSonicTokenSymbols.has(t.symbol));
      
      if (newSonicTokens.length > 0) {
        if (isFullRunInProgress) {
          pendingNewTokens.push(...newSonicTokens);
        } else {
          await scrapeNewTokens(newSonicTokens);
          newSonicTokens.forEach((t: Token) => lastScraped[t.symbol] = new Date().toISOString());
        }
      }
      lastSonicTokenSymbols = currentSonicSymbols;
    } catch (error) {
      console.error('Error processing Sonic file change:', error);
    }
  });
}
