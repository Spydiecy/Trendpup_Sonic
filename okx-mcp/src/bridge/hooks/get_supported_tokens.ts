...
export async function get_supported_tokens(chainId: string, privateKey?: string): Promise<string> {
  try {
    ...
    const tokens = await client.bridge.getSupportedTokens(chainId);
    return JSON.stringify({
      status: "success",
      data: tokens,
      message: "OKX_BRIDGE_GET_SUPPORTED_TOKENS_SUCCEEDED",
    });
  } catch (error) {
    return JSON.stringify({
      status: "error",
      error,
      message: "OKX_BRIDGE_GET_SUPPORTED_TOKENS_ERROR",
    });
  }
}